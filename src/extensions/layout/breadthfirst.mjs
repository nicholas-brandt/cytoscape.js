...
BreadthFirstLayout.prototype.run = function () {
  ...
  const maximal =
    options.acyclic || options.maximal || options.maximalAdjustments > 0; // maximalAdjustments for compat. w/ old code; also, setting acyclic to true sets maximal to true

  const hasBoundingBox = !!options.boundingBox;
  const bb = math.makeBoundingBox(
    hasBoundingBox ? options.boundingBox : structuredClone(cy.extent()),
  );
  ...
  if (directed && maximal) {
    ...
    while (Q.length > 0) {
      ...
      if (didShift) {
        ele
          .outgoers()
          .filter((el) => el.isNode() && eles.has(el))
          .forEach(enqueue);
  ...
  // average node size
  const aveNodeSize = nodes.reduce(
    (acc, node) =>
      ((box) => ({
  ...
  const maxDepthSize = depths.reduce(
    (max, eles) => Math.max(max, eles.length),
    0,
  );
      ...
      const distanceX = Math.max(
        ...
        depthSize === 1
          ...
          : // inside a bounding box, no need for left & right padding
            hasBoundingBox
            ? (bb.w - options.padding * 2 - aveNodeSize.w) /
  ...
  if (Object.keys(rotateDegrees).indexOf(options.direction) === -1) {
    util.error(
      `Invalid direction '${options.direction}' specified for breadthfirst layout. Valid values are: ${Object.keys(rotateDegrees).join(", ")}`,
    );
  }

  const getPosition = (ele) =>
    util.rotatePosAndSkewByBox(
      getPositionTopBottom(ele),
      bb,
      rotateDegrees[options.direction],
    );
