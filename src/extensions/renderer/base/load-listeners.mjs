...
BRp.load = function () {
      ...
      if (r.hoverData.which === 3) {
        ...
        if (isOverThresholdDrag) {
          if (
            ...
            const allowPassthrough = allowPanningPassthrough(
              down,
              r.hoverData.downs,
            );
      ...
      if (wheelDeltas.length >= wheelDeltaN) {
        ...
        if (inaccurateScrollDevice) {
          for (let i = 0; i < wds.length; i++) {
            inaccurateScrollFactor = Math.min(
              Math.abs(wds[i]),
  ...
  r.registerBinding(
    ...
    (touchstartHandler = function (e) {
      ...
      if (e.touches[0]) {
        const pos = r.projectIntoViewport(
          e.touches[0].clientX,
          e.touches[0].clientY,
        );
        now[0] = pos[0];
      ...
      if (capture && r.touchData.cxt) {
        ...
        if (
          ...
          if (allowPassthrough) {
            ...
            if (!r.data.bgActivePosistion) {
              r.data.bgActivePosistion = math.array2point(
