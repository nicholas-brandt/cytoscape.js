      ...
      if (nearEdge) {
        ...
        if (
          nearEdge.pstyle("z-compound-depth").value ===
            ele.pstyle("z-compound-depth").value &&
          nearEdge.pstyle("z-compound-depth").value ===
            ele.pstyle("z-compound-depth").value
        ) {
    ...
    if (
      ...
      for (let i = 0; i + 3 < pts.length; i += 2) {
        if (
          ...
          widthSq >
            (sqDist = math.sqdistToFiniteLine(
              x,
              y,
              pts[i],
              pts[i + 1],
      ...
      for (let i = 0; i + 5 < rs.allpts.length; i += 4) {
        if (
          ...
          widthSq >
            (sqDist = math.sqdistToQuadraticBezier(
              x,
              y,
              pts[i],
              pts[i + 1],
...
BRp.getAllInBox = function (x1, y1, x2, y2) {
  ...
  for (let e = 0; e < eles.length; e++) {
    ...
    if (ele.isNode()) {
      ...
      const labelBoxSelectEnabled =
        node.pstyle("box-select-labels").strValue === "yes";

      if (nodeBoxSelectMode === "none") {
        continue;
      }
      const includeLabels =
        (nodeBoxSelectMode === "overlap" || labelBoxSelectEnabled) &&
        textEvents;
      ...
      if (nodeBoxSelectMode === "contain") {
        ...
        if (labelBoxSelectEnabled && textEvents) {
          ...
          if (
            rotatedLabelBox &&
            math.satPolygonIntersection(rotatedLabelBox, selectionBox)
          ) {
            box.push(node);
            selected = true;
          }
      ...
      if (edgeBoxSelectMode === "contain") {
        if (
          rs.startX != null &&
          rs.startY != null &&
          !math.inBoundingBox(boxBb, rs.startX, rs.startY)
        ) {
          continue;
        }
        if (
          rs.endX != null &&
          rs.endY != null &&
          !math.inBoundingBox(boxBb, rs.endX, rs.endY)
        ) {
          continue;
        }
