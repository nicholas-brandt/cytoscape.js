import {
  ...
  RENDER_TARGET,
  TEX_PICKING_MODE,
} from "./drawing-elements-webgl.mjs";
...
function getBGColor(r) {
  ...
  const cssColor =
    (container && container.style && container.style.backgroundColor) ||
    "white";
...
function overrideCanvasRendererFunctions(r) {
...

  {
    ...
    r.findNearestElements = function (x, y, interactiveElementsOnly, isTouch) {
      ...
      return findNearestElementsWebgl(
        r,
        x,
        y,
...
function drawSelectionRectangle(r, options) {
  r.drawSelectionRectangle(options, (context) =>
  ...
  if (r.webglDebug) {
    ...
    if (compact) {
      ...
      for (let info of atlasInfo) {
        console.log(
          `  ${info.type}: ${info.keyCount} keys, ${info.atlasCount} atlases`,
        );
      }
      console.log("");
    }
